...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Функция создания слайдов для филиала
    function createBranchSlides(branchName, tabSafeBranchName) {
        ...
        // Слайд с наймом/увольнениями
        slidesHtml += `
            <div class="slide-card">
                <div class="slide-header">
                    <i class="bi bi-graph-up me-2"></i>Динамика найма и увольнений
                </div>
                <div class="slide-content">
                    <div class="row">
                        <div class="col-md-8">
                            <div class="chart-container">
                                <canvas id="hiring-chart-${tabSafeBranchName}" style="max-height: 300px;"></canvas>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <h6>Период отчета: ${hiringSlide.vacancies_report_period}</h6>
                            
                            <div class="measures-section">
                                <h6 class="text-success">Сделать и выполнить:</h6>
                                <ul class="measures-list">
                                    ${hiringSlide.improvement_measures.take_and_do.map(measure => `<li>${measure}</li>`).join('')}
                                </ul>
                            </div>
                            
                            <div class="measures-section">
                                <h6 class="text-warning">Нужны ресурсы:</h6>
                                <ul class="measures-list">
                                    ${hiringSlide.improvement_measures.need_resources.map(measure => `<li>${measure}</li>`).join('')}
                                </ul>
                            </div>
                            
                            <div class="measures-section">
                                <h6 class="text-info">Системно на уровне компании:</h6>
                                <ul class="measures-list">
                                    ${hiringSlide.improvement_measures.systemic_company_level.map(measure => `<li>${measure}</li>`).join('')}
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        // Слайд с Exit-интервью
        slidesHtml += `
            <div class="slide-card">
                <div class="slide-header">
                    <i class="bi bi-chat-dots me-2"></i>Анализ Exit-интервью
                </div>
                <div class="slide-content">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <ul class="measures-list">
                                ${exitSlide.recommendations.map(rec => `<li>${rec}</li>`).join('')}
                            </ul>
                        </div>
                        <div class="col-md-6">
                                    ...
                                    <div class="atmosphere-item">
                                        ...
                                        <div class="progress mb-2" style="height: 25px;">
                                            <div class="progress-bar progress-bar-good" role="progressbar" 
                                                ...
                                                ${Math.round(data.good_percent)}% положительно
                                            </div>
                                            <div class="progress-bar progress-bar-bad" role="progressbar" 
                                                ...
                                                ${Math.round(data.bad_percent)}% отрицательно
                                            </div>
                                        </div>
    ...
    function createExitReasonsChart(branchName, tabSafeBranchName) {
        ...
        if (!canvas) {
            console.log(`Canvas not found: #exit-reasons-chart-${tabSafeBranchName}`);
            return;
        }

        const branchData = APP_DATA.data[branchName];
        if (!branchData || !branchData.slides || !branchData.slides.exit_interviews) {
            console.log(`No exit interview data for branch: ${branchName}`);
            if (noDataDiv) {
        ...
        if (!topReasons || topReasons.length === 0) {
            console.log(`No top reasons data for branch: ${branchName}`);
            if (noDataDiv) {
                canvas.style.display = 'none';
                noDataDiv.style.display = 'block';
            }
        ...
        console.log(`Creating exit reasons chart for ${branchName}:`, topReasons);
        
        // Скрываем сообщение "Данные отсутствуют" и показываем canvas
        if (noDataDiv) {
        ...
        const colors = [
            '#6f42c1', // Фиолетовый (как на изображении)
            '#17a2b8', // Голубой
            '#28a745', // Зеленый
            '#ffc107', // Желтый
            '#fd7e14', // Оранжевый
        ...
        try {
            new Chart(canvas, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            display: function(context) {
                                ...
                                return percentage > 8; // Показывать только если больше 8%
                            },
                            color: '#fff',
                            font: {
                                weight: 'bold',
                    ...
                    cutout: '40%', // Создает эффект пончика
                    animation: {
                        animateRotate: true,
                        animateScale: false
                    }
            ...
            console.log(`Exit reasons chart created successfully for ${branchName}`);
        } catch (error) {
    ...
    function openBranchTab(branchName){
        console.log(`Opening branch tab for: ${branchName}`);
        console.log(`Branch data:`, APP_DATA.data[branchName]);
        ...
        // Создание контента для вкладки с таблицей, графиком и слайдами
        tabPane.innerHTML= '<div class="d-flex justify-content-between align-items-center flex-wrap mb-2">' + '<h5>' + branchName + ' - Детализация</h5>' + '<button class="btn btn-sm btn-outline-success branch-export-btn" id="export-excel-branch-' + tabSafeBranchName + '"><i class="bi bi-file-earmark-excel"></i> Excel</button>' + '</div>' + '<div class="row g-2 mb-3 mt-0"><div class="col-md-4"><select id="year-select-'+tabSafeBranchName+'" multiple placeholder="Год"></select></div><div class="col-md-4"><select id="quarter-select-'+tabSafeBranchName+'" multiple placeholder="Квартал"></select></div><div class="col-md-4"><select id="month-select-'+tabSafeBranchName+'" multiple placeholder="Месяц"></select></div></div>' + '<div class="cb-bar mb-2" id="kpi-cb-bar-'+tabSafeBranchName+'"></div>' + '<div class="table-responsive-lg tbl-wrap mb-3"><table id="table-'+tabSafeBranchName+'" class="table table-bordered table-hover table-sm branch-pane-table"><thead></thead><tbody></tbody></table></div>' + '<div class="chart-container"><canvas id="chart-'+tabSafeBranchName+'"></canvas></div>' + createBranchSlides(branchName, tabSafeBranchName);
